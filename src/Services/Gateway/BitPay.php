...
class BitPay extends AbstractPayment
{
    ...
    public function MetronPay($type, $price, $shopinfo, $paylist_id=0)
    {
        ...
        $data           = [
            'merchant_order_id' => $pl->tradeno,
            'price_amount'      => (float) $pl->total,
            'price_currency'    => 'CNY',
            'title'             => '支付单号：' . $pl->tradeno,
            'description'       => '充值：' . $pl->total . ' 元',
            'callback_url'      => $_ENV['baseUrl'] . '/payment/notify/bitpay',
            'success_url'       => $_ENV['baseUrl'] . '/user/payment/return?tradeno='.$pl->tradeno,
            'cancel_url'        => $_ENV['baseUrl'] . '/user',
            'token'             => $this->sign($str_to_sign),
            'checkout'          => ($type === 'Alipay' ? 'QBIT' : $type),
        ];
    ...
    public function notify($request, $response, $args)
    {
        ...
        if ($resultVerify && $isPaid) {
            $this->postPayment($data['merchant_order_id'], '数字货币');
            // echo 'SUCCESS';
            $return = [];
            $return['status'] = 200;
            echo json_encode($return);
