...
class BobFacePay extends AbstractPayment
{
    ...
    private function createGateway()
    {
        ...
        $gateway->setAppId(self::pay_config('alipay_app_id'));
        $gateway->setPrivateKey(self::pay_config('merchant_private_key')); // 可以是路径，也可以是密钥内容
        $gateway->setAlipayPublicKey(self::pay_config('alipay_public_key')); // 可以是路径，也可以是密钥内容
        $notifyUrl = $_ENV['baseUrl'] . '/api/v1/bob/payment/notify';
    ...
    public function purchase($request, $response, $args)
    {
        ...
        $return['ret'] = 1;
        $return['url'] = $_ENV['baseUrl'].'/api/v1/qrcode?price='.$price.'&qrcode='.$qrCodeContent.'&tradeno='.$pl->tradeno.'&type=alipay';
    ...
    {
        file_put_contents(BASE_PATH . '/storage/bobpay.log', json_encode($_POST) . "\r\n", FILE_APPEND);
